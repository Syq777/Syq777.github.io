...
test("首页列出全部周报并优先显示最新一期", async () => {
  ...
  assert.match(html, /href="\/weekly\/005\/"/);
...
test("每一期都有独立静态页面和正确的正文", async () => {
  ...
  const fifth = await readFile(new URL("weekly/005/index.html", root), "utf8");
  ...
  assert.match(fifth, /服贸会展馆/);
  assert.match(fifth, /width="2400" height="1800"/);
  assert.match(fifth, /rel="canonical" href="https:\/\/syq777\.github\.io\/weekly\/005\/"/);
});
...
test("发布所需资源完整", async () => {
  await Promise.all([
    ...
    access(new URL("uploads/005/shougang-park.jpg", root)),
    access(new URL("uploads/005/ciftis-exhibition-hall.jpg", root)),
  ]);
});
